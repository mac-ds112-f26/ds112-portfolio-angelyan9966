...
ggplot(elections, aes(x = repub_pct_20)) +
  geom_density() +
  labs(x = "repub_votes", y = "numbers of votes")
```
...
```
#comment: both are not a good data, but median_rent is better since more data are concertrated in a space, median_age is more spread out
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point()
```
There are so many points are overlapped, and hard to compare the distributions.
...
All of the plots are showing a wide range of Republican support in 2020, red states have a much higher value compared to blue states. Purple states falls between them.
...
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```
#comment: color is false, and density is overlapped.
...
```
#comment: need to write the code  scale_fill_manual(values = c("blue", "purple", "red")
...
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  ...
  scale_fill_manual(values = c("blue", "purple", "red"))
```
#comment: it looks like the opacity is 50%, alpha is transparency.
...
```
#comment: it separate the density into 3 categories and panels.
...
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  ...
  scale_fill_manual(values = c("blue", "purple", "red"))
```
#comment: it's hard to compare the shape and distribution directly.
...
I will choose density plots since it has more details about the shape of the distribution. But boxplots can make outliers easier to see.
...
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
```
#comment: historical is the x-axis, winner_20 is the color fills the bar.
...
ggplot(elections, aes(x = winner_20)) +
  ...
  facet_wrap(~historical)
```
#comment: winner_20 is the x-axis, historical creates the facet to see blue, purple and red states.
...
```
#comment: "dodge" makes the Democratic and Republican bars side by side, it can helps us compare these parties more directly.
...
```
#commentL position = "fill" makes every bar the same height and shows proportions.

#### Part a

Name one pro and one con of using the "proportional bar plot" instead of one of the other three options.
pro: it's easy to compare the relative proportions, even the total numbers of votes are different.
con: it hides the excat numbers that we cannot tell the actual numbers.

#### Part b

What's your favorite bar plot from part and why?
Maybe side-by-side bar plot because it makes it easy to directly compare the number of different categories.
...
- categorical, bar chart. if one more than add position = "fill"
...
- there are so many states in one plot, it's hard to see and distinguish the color, which led to hard to compare.
...
ggplot(elections, aes(x = repub_pct_20)) + 
  geom_density(alpha = 0.5, color = "pink", fill = "green") + 
...
# THINK: What DON'T you like about this? There are so many states, hard to compare
ggplot(elections, aes(x = repub_pct_20, y = state_abbr)) + 
  geom_density_ridges(color = "orange", fill = "blue")
```
...
ggplot(elections, aes(x = repub_pct_20, y = fct_reorder(state_abbr, repub_pct_20))) + 
  geom_density_ridges(alpha = 0.5, color = "yellow", fill = "orange")

```

**Follow-up questions**

-   Which states tend to have the most variability in outcomes from county to county? The least? most:NE, least:DC
-   What other interesting patterns do you notice? some peaks are strong.
-   Does this plot prompt any other questions? how large difference between the states?
